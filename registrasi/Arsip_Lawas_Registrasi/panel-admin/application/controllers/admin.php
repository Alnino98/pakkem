...
class Admin extends CI_controller
{
  ...
  {
   $x = array('judul' =>':: Data Anggota ::',
              'data'  =>$this->m_admin->anggota(),);
     tpl('admin/anggota',$x);
  }
	...
	{
	  $sql=$this->db->get_where('mst_anggota',array('mst_no_pendaftaran'=>$id))->row_array(); 
	  $x = array(
			'judul'        		=>'View Data Anggota' ,
			'aksi'        		=>'edit',
			'mst_nama'			=>$sql['mst_nama'],
			'email'			=>$sql['email'],
			'namafoto'			=>$sql['namafoto'],
			'kantor'    		=>$sql['kantor']); 
...
public function anggota_hapus($id='')
  {
    $foto=$this->db->get_where('mst_anggota',array('mst_no_pendaftaran'=>$id))->row_array();
    if($foto['namafoto'] != ""){ @unlink('template/data/'.$foto['namafoto']); }else{ }

    $cek=$this->db->delete('mst_anggota',array('mst_no_pendaftaran'=>$id));
   if ($cek) {
    $pesan='<div class="alert alert-success alert-dismissible">
                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                <h4><i class="icon fa fa-check"></i> Success!</h4>
